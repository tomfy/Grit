  ...
  //  ****************** start iterating chains ******************
    ...
    do{  // once through this loop does 1 update to each of N_temperatures*N_chain chains
		...
		// check_path_consistency_multi(the_path); // getchar();
                ...
                if(r_in->Output_paths  && iii==0 && (the_path->L <= 28 && the_path->L_t <= 11) && (steps_so_far % 100 == 0)) {
		  sprintf(path_filename, "%spaths.out", output_prefix);
		  FILE* fp_paths  = fopen(path_filename, "w");
		  print_path_brief(fp_paths, the_path); // print_path(fp_paths, the_path);
		}
	...
	if(steps_so_far - last_print_n_steps >= 10 && steps_so_far > 1.02*(double)last_print_n_steps){ // print some stuff to stdout
	  ...
	  last_print_n_steps = progress_output(r_in, n_mc3_swap, the_chain_set, steps_so_far, n_mc3_swap_try, tstart);
        }
...
long progress_output(const Run_info_in* r_in, int* n_mc3_swap, Chain_set* the_chain_set, long steps_so_far, long n_mc3_swap_try, long tstart)
{ // print some stuff to pstream (global)
  long tnow = time(NULL); //GETCPUTIME;
  ...
  if(r_in->N_temperatures > 1) {
    fprintf(PROGSTREAM, "    MCMCMC swaps: ");
    for(long iii=0; iii<r_in->N_temperatures-1; iii++){
  ...
  fprintf(PROGSTREAM, "     T,   Avg:   L,        L_i,      L_t,      lambdaI,    lambdaT \n");
  for(long iii=0; iii<r_in->N_temperatures; iii++){
    fprintf(PROGSTREAM, "%8g      %8g  %8g  %8g  %10g  %10g\n", the_chain_set->chain[0][iii]->temperature, 
  ...
  fprintf(PROGSTREAM, "     T,   B/W:   L,        L_i,      L_t,      lambdaI,    lambdaT \n"); 
  for(long iii=0; iii<r_in->N_temperatures; iii++){
    fprintf(PROGSTREAM, "%8g      %8g  %8g  %8g  %10g  %10g\n", the_chain_set->chain[0][iii]->temperature, 
	...
	fprintf(PROGSTREAM, "*************  Burn-in done at %i updates.  ****************** \n", steps_so_far);
	//  getchar();
      }
    }
  }
                
  fprintf(PROGSTREAM, "    L_min: \n");
  for(long iii=0; iii<r_in->N_temperatures; iii++){
    for(long ii=0; ii<r_in->N_chain; ii++){ fprintf(PROGSTREAM, " %3i ", the_chain_set->chain[ii][iii]->L_min);} fprintf(PROGSTREAM, "\n");
  } // printf("\n");
  fprintf(PROGSTREAM, "    L_t_min: \n");
  for(long iii=0; iii<r_in->N_temperatures; iii++){
    for(long ii=0; ii<r_in->N_chain; ii++){ fprintf(PROGSTREAM, " %3i ", the_chain_set->chain[ii][iii]->L_t_min);} fprintf(PROGSTREAM, "\n");
  } // printf("\n");
